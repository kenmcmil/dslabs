...
public final class ScalarClockAppendTest extends BaseJUnitTest {
    ...
    public static StateGeneratorBuilder builder() {
        ...
        builder.serverSupplier(a -> new ScalarClockAppendServer(a,a.equals(server(1)) ? server(2) : server(1)));
        ...
        StatePredicate showResultsMatch =
            statePredicate("Replicas consistent when network quiescent",
                           s -> {
                               ...
                               if (!msgs)
                                   for (int i = 0; i < numServers; i++) {
                                       ScalarClockAppendServer si = (ScalarClockAppendServer) s.server(servers.get(i));
                                       Result r = si.app.execute(cmd);
                                       if (!(r instanceof GetResult))
                                           return false;
                                       String vi = ((GetResult)r).value();
                                       for (int j = i+1; j < numServers; j++) {
                                           ScalarClockAppendServer sj = (ScalarClockAppendServer) s.server(servers.get(j));
    ...
    public void test04TwoServerMessagePerAppend() throws InterruptedException {
        ...
        runSettings.addInvariant(twoServerMessagePerAppend);
        runState.run(runSettings);
    }
    ...
    public void test06ResultsOkNonconcurrent() throws InterruptedException {
        ...
        for (int j = 0; j < 2; j++) {
            ...
            runState.clientWorker(clients.get(j)).addCommand(new Append("key",String.valueOf(j)),new AppendResult(val));
            runState.clientWorker(clients.get(j)).addCommand(new Get("key"),new GetResult(val));
            runState.run(runSettings);
        }
     }
